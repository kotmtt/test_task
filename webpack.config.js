...
const config = {
    ...
    entry: () => {
        ...
        if (isTwigLoader && isDevelopment) {
            glob
                ...
                .forEach(twigFile => {
                    entries['index'].push(path.join(__dirname, './src/pages/', twigFile));
                });
        }

        glob.sync(__paths.css.main)
            .forEach(filePath => {
                const name = path.parse(filePath).name;
                entries[name] = entries[name] || [];
                entries[name].push(path.resolve(__dirname, filePath));
        ...
        // Js компонентов
        glob.sync(__paths.js.components.src)
            .forEach(filePath => {
    ...
    module: {
        rules: [
            {
                test: /\.js$/,
                exclude: /(node_modules\/(?!(swiper|dom7|micromodal|sticky-sidebar|parsleyjs)\/).*|bower_components)/,
                use: [
                    {
                        loader: 'babel-loader',
                        options: {
            ...
            {
                test: /\.(png|jpg|gif|webp|svg|woff|woff2|eot|ttf|otf)$/,
                use: [
                    {
                        loader: 'file-loader',
                        options: {
                ...
                use: [
                    'raw-loader',
                    {
                        ...
                        options: {
                            ...
                            extend: (Twig) => {
                                Twig.exports.extendTag(require('./tasks/viewTag')(Twig));
                                Twig.exports.extendTag(require('./tasks/svgTag')(Twig));
                            },
                            namespaces: {
                                layouts: './src/include/layout',
                                ui: './src/include/@ui',
                                molecules: './src/include/^components',
                                organisms: './src/include/&organisms',
                                landing: './src/include/_landing',
                                email: './src/include/#email',
                            },
                        }
                    }
                ]
